...
static char *BRCAddress(int32_t lat, int32_t lon)
{
    ...
    float bearingToMan = GeoCoord::bearing(BRC_LATF, BRC_LONF, DegD(lat), DegD(lon)) * RAD_TO_HOUR;
    bearingToMan += 12.0 - BRC_NOON;
    while (bearingToMan > 12.0) {
        bearingToMan -= 12.0;
    }
    ...
    float d = GeoCoord::latLongToMeter(BRC_LATF, BRC_LONF, DegD(lat), DegD(lon)) * METER_TO_FEET;
    ...
    if (bearingToMan > 1.75 && bearingToMan < 10.25) {
        ...
        for (auto &&m :
             {std::tuple<const float, const float, const char *>{
                  2500, 50, "Esp"}, // Esp's center is 2500ft  from man, start showing Esp 50ft from center of street.
